import 'package:flutter/material.dart';

/// Application color palette
class AppColors {
  AppColors._();

  // Primary Colors
  static const Color primary = Color(0xFF111111);
  static const Color primaryVariant = Color(0xFF2B2B2B);
  static const Color secondary = Color(0xFFF2F2F2);
  static const Color secondaryVariant = Color(0xFFBDBDBD);

  // Background Colors
  static const Color background = Color(0xFFFFFFFF);
  static const Color surface = Color(0xFFFFFFFF);
  static const Color error = Color(0xFFB00020);

  // Text Colors
  static const Color onPrimary = Color(0xFFFFFFFF);
  static const Color onSecondary = Color(0xFF111111);
  static const Color onBackground = Color(0xFF111111);
  static const Color onSurface = Color(0xFF111111);
  static const Color onError = Color(0xFFFFFFFF);

  // Grey Shades
  static const Color grey100 = Color(0xFFF5F5F5);
  static const Color grey200 = Color(0xFFEEEEEE);
  static const Color grey300 = Color(0xFFE0E0E0);
  static const Color grey400 = Color(0xFFBDBDBD);
  static const Color grey500 = Color(0xFF9E9E9E);
  static const Color grey600 = Color(0xFF757575);
  static const Color grey700 = Color(0xFF616161);
  static const Color grey800 = Color(0xFF424242);
  static const Color grey900 = Color(0xFF212121);

  //BNPL Colors
  static const Color bnplPrimary = Color.fromARGB(255, 205, 176, 246);
}
